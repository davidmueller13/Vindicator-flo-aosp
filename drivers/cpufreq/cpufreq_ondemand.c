				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(od_cpu_dbs_info, j);
		dbs_info->prev_cpu_idle = get_cpu_idle_time_io(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time_io(j, &cur_wall_time, dbs_tuners_ins.io_is_busy);
...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < policy->max) {
		...
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time_io(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time_io(j,
